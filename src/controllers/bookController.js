...
const createBook = async (req, res) => {
  try {
    ...
    if (!title || title.trim() == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book title" });
    ...
    if (excerpt != undefined && typeof (excerpt) != "string") {
      return res
        .status(400)
        .send({ status: false, message: "plese enter valid excerpt" });
    }
    if (!excerpt || excerpt.trim() == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book excerpt" });
   

    if (!userId || userId == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book userId" });
    ...
    if (ISBN != undefined && typeof (ISBN) != "string") {
      return res
        .status(400)
        .send({ status: false, message: "plese enter valid ISBN" });
    }
    if (!ISBN || ISBN.trim() == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book ISBN" });
    ...
    if (category != undefined && typeof(category) != "string") {
      return res
        .status(400)
        .send({ status: false, message: "plese enter valid category" });
    }
    if (!category || category.trim() == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book category" });

    if (subcategory != undefined && typeof(subcategory) != "string") {
      return res
        .status(400)
        .send({ status: false, message: "plese enter valid subcategory" });
    }
    if (!subcategory || subcategory.trim() == "")
      return res
        .status(400)
        .send({ status: false, message: "Please enter book subcategory" });
...
const getBooks = async (req, res) => {
  ...
  if (Object.keys(data).length == 0) {
    let foundData = await bookModel
      .find({ isDeleted: false })
      .select({ title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1 });
  ...
  if (count > 0)
    return res
      ...
      .send({
        status: false,
        message: "queries can only have userId,category and subcategory",
      });
  ...
  if (foundData.length == 0)
    return res
      .status(404)
      .send({ status: false, message: "No data found with given parameters" });
  foundData = foundData.sort((a, b) => a.title.localeCompare(b.title));
  return res.status(200).send({ status: true, message: foundData });
};
...
const getBookById = async function (req, res) {
  try {
    ...
    if (!bookId)
      return res
        .status(400)
        .send({ status: false, message: "Blog Id Is Needed" });
    ...
    if (!bookData)
      return res
        .status(404)
        .send({ status: false, msg: "No Book Found With Given Book Id" });
    ...
    return res
      .status(200)
      .send({ status: "true", message: "Booklist", data: data });
  } catch (error) {
  ...
  try {
    ...
    if (!bookId || bookId.trim() == "") {
      return res
        ...
        .send({
          status: false,
          message: `please provide bookId in order to update..`,
        });
    }
    if (!mongoose.isValidObjectId(bookId)) {
      return res
        ...
        .send({
          status: false,
          message: `please provide valid bookId in order to update..`,
        });
    }

    let isBookExist = await bookModel
    ...
    if (!isBookExist) {
      return res
        ...
        .send({
          status: false,
          message: `sorry there is no book in database with provided bookId..`,
        });
    }
    ...
    if (count > 0) {
      return res
        ...
        .send({
          status: false,
          message: `you can only update ${validQueries} attributes `,
        });
    }
    ...
    if (isTitleExist) {
      return res
        ...
        .send({
          status: false,
          message: `plese check your title as there is already one document with same title...`,
        });
    }
    if (isISBNExits) {
      return res
    ...
    let updatedData = await bookModel.findOneAndUpdate({ _id: bookId }, query, {
      new: true,
    });
    return res
      .status(200)
...
const deleteById = async (req, res) => {
  try {
    ...
    if (!foundBook)
      return res
        .status(404)
        .send({ status: false, message: "No Data Found to Delete" });
    ...
    foundBook = await bookModel.findOneAndUpdate(
      { _id: bookId },
      { $set: { isDeleted: true } }
    );
